...
let errorHandler:object = {};
const isDev:boolean = process.env.NODE_ENV === 'development';
const baseURL = isDev ? 'http://121.89.205.189/admin' : 'http://121.89.205.189/admin'
...
ins.interceptors.response.use(function(response: AxiosResponse<any>): AxiosResponse<any> {
  ...
  if (response.data.code !== '200') {
    if (response.data.code === '4' || response.data.code === '10005') {
      if (errorHandler[response.data.code as keyof typeof errorHandler] && response.data.code === '10005') {
        return new Promise(() => {})
      }
